...
inline void zero(double* matrix, std::size_t size)
{
...
inline void eye(double* matrix, std::size_t size)
{
  zero(matrix, size);
  for (std::size_t i = 0; i < size; ++i) {
...
inline void random_matrix(double* matrix, std::size_t size, double min, double max)
{
...
inline void print_matrix(std::string name, double* matrix, std::size_t size)
{
  printf("%s = ", name.c_str());
  std::printf("[");
